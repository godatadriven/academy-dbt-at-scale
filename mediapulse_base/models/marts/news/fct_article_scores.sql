with articles as (
    ...
    select * from {{ ref('int_news__articles_deduped') }}
...
--   2. coalesce missing scores/response counts to 0 so they don't null out the whole sum
...
scored as (
    ...
    select
        ...
        coalesce(score_bias, 0) * 2 * coalesce(num_responses_bias, 0)
            as weighted_bias,

        -- trust: scale is 0-100, rescale to 0-10 by dividing by 10
        (coalesce(score_trust, 0) / 10.0) * coalesce(num_responses_trust, 0)
            as weighted_trust,
        ...
        coalesce(score_engagement, 0) * coalesce(num_responses_engagement, 0)
        ...
        coalesce(num_responses_relevance, 0)
            ...
            + coalesce(num_responses_engagement, 0)
            as total_responses
...
final as (
    ...
    select
        ...
        round(
            (weighted_relevance + weighted_clarity + weighted_bias + weighted_trust + weighted_engagement)
