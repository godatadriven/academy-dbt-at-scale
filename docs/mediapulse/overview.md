...
**MediaPulse** is a fictional media conglomerate that owns four distinct platforms. Each platform generates its own raw data, and all four feed into a shared dbt project that your group will work on today.
...
|----------|-------------|------------|
| **StreamVault** | Subscription streaming service — films, series, live sport | `raw_streaming` |
| **NewsNow** | Digital news outlet — articles, authors, page views | `raw_news` |
| **PodcastHub** | Podcast network — shows, episodes, listener events | `raw_podcasts` |
| **AdConnect** | Programmatic ad platform — campaigns, impressions, spend | `raw_ads` |
...
??? bug "Bug 1 — `stg_news__articles.sql`: missing deduplication"
    The `raw_news.articles` table contains duplicate `article_id` values because articles can be republished with an updated `updated_at` timestamp. The staging model selects all rows without deduplication, causing downstream fans when joined.
