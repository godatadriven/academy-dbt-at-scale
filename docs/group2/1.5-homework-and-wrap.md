...
- [ ] Run dbt Wizard against `mediapulse_base` and get a prioritized coverage-gap list — start with the streaming domain's staging layer, which doesn't have a `models.yml` yet
- [ ] Fill the identified gaps using dbt Wizard, reviewing every suggestion before accepting it (don't accept a `unique` test on a column you haven't confirmed is actually unique)
...
- [ ] Open `mediapulse_base/macros/generate_schema_name.sql` and identify its two branches (seeds under `_seeds_setup` vs. everything else) — compare against dbt's default `generate_schema_name` macro to see exactly what's being overridden and why
