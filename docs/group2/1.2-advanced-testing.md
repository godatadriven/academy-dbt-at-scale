...
Create a new branch call `dpg/<your_name>/day1`. You will commit all your changes to this branch.
...
3. Run `dbt build` and confirm the gaps you picked are actually closed.
...
`int_dedupe_subscribers` claims to collapse the subscriptions table down to one row per user. That's a testable claim about row counts, not just column values - add a `dbt_utils.fewer_rows_than` test comparing `int_dedupe_subscribers` against the staging model it's built from. If this test ever passes with equal or more rows, the dedup logic has silently stopped working.
...
That fix isn't free, though: with the old join, a watch event could in principle tell you which subscription was active when it happened (modulo the missing date filter it never had). With `int_dedupe_subscribers`, every event for a user gets stamped with that user's single *most recent* subscription, regardless of which one was actually active at `watched_at`. 
