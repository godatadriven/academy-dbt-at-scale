...
You already know the four generic tests, and dbt Wizard as a way to draft them fast. This topic is about closing coverage gaps at scale, then reasoning about the *parameters* generic tests include - `where`, `severity`, `compare_model` - instead of reaching for the first default that fits. 
