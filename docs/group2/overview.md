...
1. [dbt Catalog & review](1.1-dbt-catalog-and-review.md) - a brisk recap of Catalog on `mediapulse_platform`, then a full mesh-diagnosis exercise using Catalog's cross-project lineage view
...
- `mediapulse_platform` - the producer project; note its `access` settings in `dbt_project.yml` and its `stg_news__authors.email` column
